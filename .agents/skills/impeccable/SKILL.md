---
name: impeccable
description: 打造有辨识度、可投产级设计质量的前端界面；生成有创意且经过打磨的代码，避免千篇一律的「AI 通用审美」。在用户要求构建 Web 组件、页面、物料、海报或应用时，或任一设计技能需要项目上下文时使用。使用 craft 表示先塑形再实现，teach 表示建立设计上下文，extract 表示将可复用组件与设计令牌抽离到设计体系中。
version: 2.1.1
user-invocable: true
